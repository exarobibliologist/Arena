...
class ArenaGame:
    ...
    def preparation_phase(self, gladiator: Gladiator, points: int) -> None:
        if gladiator.is_player:
            while points > 0:
                self.render_arena()
                choice = self.gui_input("PREP", {"points": points, "atk": gladiator.base_atk, "def": gladiator.base_def_stat})
                if choice == '1':
                    ...
                    points -= 1
            self.gui_input("MESSAGE", f"PREPARATION COMPLETE\n\nFinal Base Attack: {gladiator.base_atk}\nFinal Base Defense: {gladiator.base_def_stat}")
        else:
